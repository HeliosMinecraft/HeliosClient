...
public class BlockListSettingsScreen extends Screen implements IWindowContentRenderer {
    private final Window window;
    private final BlockListSetting setting;
    int windowWidth = 258, windowHeight = 140;
    private final SearchBar searchBar;

    public BlockListSettingsScreen(BlockListSetting setting) {
    ...
    public void renderContent(Window window, DrawContext drawContext, int x, int y, int mouseX, int mouseY) {
        ...
        if (setting != null) {
            windowHeight = setting.handleBlocksRendering(x + 1, y + 30, mouseX,mouseY, drawContext,windowWidth);
