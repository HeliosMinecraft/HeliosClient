...
import dev.heliosclient.module.settings.ParticleListSetting;
...
public class ParticleListSettingsScreen extends Screen implements IWindowContentRenderer {
    private final Window window;
    private final ParticleListSetting setting;
    int windowWidth = 258, windowHeight = 140;
    private final SearchBar searchBar;

    public ParticleListSettingsScreen(ParticleListSetting setting) {
    ...
    public void renderContent(Window window, DrawContext drawContext, int x, int y, int mouseX, int mouseY) {
        ...
        if (setting != null) {
            windowHeight = Math.round(setting.handleParticlesRendering(x + 1, y + 30, drawContext,windowWidth));
            window.setWindowHeight(windowHeight + 35);
        }
    }
}
