...
public class ChatHighlight extends Module_ {
    ...
    public void checkForHighlights() {
        ...
        // Add non-empty words to the highlight list
        ...
        // Add player's username if highlightUsername is enabled
        if (highlightUsername.value) {
            this.highlightList.add(mc.getSession().getUsername());
        }

        // Add friends' names if highlightFriends is enabled
    ...
    public void onChatMessageEvent(ChatMessageEvent event) {
        ...
        // Get the formatting color code for highlighted words from the player.
        Formatting formatting = Formatting.byName(colors.getOption().toString());

        // This should not happen, but just in case.
        if (formatting == null) return;

        String colorCode = ColorUtils.colorChar + formatting.getCode();
        ...
        for (String wordToHighlight : this.highlightList) {
            ...
            String casedMessage = applyCaseCheck(builder.toString());

            // Change the case of the word.
            String modifiedWord = applyCaseCheck(wordToHighlight);

            // Case-insensitivity check
            if (casedMessage.contains(modifiedWord)) {
                ...
                int startIndex = casedMessage.indexOf(modifiedWord);
